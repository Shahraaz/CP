...
public:
	double overlap(vector<string> polygon1, vector<string> polygon2)
	{
		...
		// printPolygon(p);
		// printPolygon(q);
		Polygon answer = GetIntersectionOfPolygons(p, q);
		return area(answer);
	}
};
...
// 	int n, m, x, y;
// 	cin >> n;
// 	vector<string> polya, polyb;
// 	for (int i = 0; i < n; ++i)
// 	{
// 		cin >> x >> y;
// 		polya.pb(to_string(x) + ' ' + to_string(y));
// 	}
// 	cin >> m;
// 	for (int i = 0; i < m; ++i)
// 	{
// 		cin >> x >> y;
// 		polyb.pb(to_string(x) + ' ' + to_string(y));
// 	}
// 	ConvexPolygons C;
// 	cout << C.overlap(polya, polyb) << '\n';
// 	return 0;
// }